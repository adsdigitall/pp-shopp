              ...
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="space-y-1 sm:col-span-2">
                      <label className="flex flex-wrap items-center gap-2 text-xs font-semibold text-[var(--text-secondary)]">Comissão mínima
                        <Input type="number" min="0" max="50" step="1" value={automation.minCommissionRate ?? 10} onChange={e => setAutomation((prev: any) => ({ ...prev, minCommissionRate: Math.min(50, Math.max(0, Number(e.target.value) || 0)) }))} className="h-7 w-14 rounded-lg border-[var(--border)] bg-[var(--surface-elevated)]" />
                        <span className="text-[9px] font-normal text-[var(--text-secondary)]">%</span>
                      </label>
                      <p className="text-[9px] leading-3 text-[var(--text-secondary)]">Produto que paga menos que isso não entra na fila automática, nem que seja campeão de vendas. Sem nada acima do corte, o Radar não manda nada naquele ciclo. 0 desliga o corte.</p>
                    </div>
                    <div className="space-y-1 sm:col-span-2">
