    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#fff7ed_0%,#f8fafc_42%,#eef2ff_100%)] flex flex-col pb-24 sm:pb-16 text-slate-900 font-sans">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-3.5 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-5 sm:space-y-6 md:ml-72">
        ...
        <div className="rounded-3xl border border-orange-100 bg-white/75 p-4 shadow-sm backdrop-blur-md">
          ...
          <div className="mt-4 flex gap-2"><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void loadProducts(false, true); }} placeholder="Buscar produto ou colar link (Shopee/Mercado Livre)" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs text-slate-700 outline-none focus:border-orange-400" /><button type="button" onClick={() => loadProducts(false, true)} className="rounded-xl bg-[#EE4D2D] px-5 py-2.5 text-xs font-black text-white hover:bg-orange-600">🔎 Garimpar</button></div>
        </div>
