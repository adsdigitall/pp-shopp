    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#fff7ed_0%,#f8fafc_42%,#eef2ff_100%)] flex flex-col pb-24 sm:pb-16 text-slate-900 font-sans">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-3.5 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-5 sm:space-y-6 md:ml-72">
        ...
        <div className="space-y-4">
          ...
          {garimparTab === 'categorias' && <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3"><span className="col-span-full text-xs font-bold text-slate-500">Escolha uma categoria para buscar ofertas reais:</span>{[['🏠','Casa'],['🍳','Cozinha'],['💻','Eletrônico'],['👗','Moda'],['💄','Beleza'],['🛠️','Ferramenta'],['⚽','Esporte'],['🐾','Pet'],['👶','Bebê']].map(([emoji,label]) => <button key={label} type="button" onClick={() => { setActiveCategory(label.toLowerCase()); setSearchQuery(''); setGarimparTab('buscar'); }} className="rounded-2xl border border-slate-200 bg-white px-3 py-3 text-left text-xs font-bold text-slate-600 hover:border-orange-300">{emoji} {label}<span className="mt-1 block text-[10px] font-medium text-slate-400">Buscar agora →</span></button>)}</div>}
