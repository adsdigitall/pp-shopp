    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#fff7ed_0%,#f8fafc_42%,#eef2ff_100%)] flex flex-col pb-24 sm:pb-16 text-slate-900 font-sans">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-3.5 sm:px-6 lg:px-8 pt-4 sm:pt-6 space-y-5 sm:space-y-6 md:ml-72">
        ...
        <div className="rounded-3xl border border-orange-100 bg-white/75 p-4 shadow-sm backdrop-blur-md">
          ...
          <div className="mt-4 flex flex-wrap gap-2">{[['buscar','Buscar'],['categorias','Categorias'],['mais-buscados','Mais buscados'],['lojas','Lojas favoritas'],['links','Por links']].map(([id,label]) => <button key={id} type="button" onClick={() => setGarimparTab(id as typeof garimparTab)} className={`rounded-xl px-3 py-2 text-xs font-bold ${garimparTab === id ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-600'}`}>{label}</button>)}</div>
          {garimparTab === 'categorias' && <div className="mt-3 flex flex-wrap items-center gap-2"><span className="text-xs font-bold text-slate-500">Categorias:</span>{[['🏠','Casa'],['🍳','Cozinha'],['💻','Eletrônico'],['👗','Moda'],['💄','Beleza'],['🛠️','Ferramenta'],['⚽','Esporte'],['🐾','Pet'],['👶','Bebê']].map(([emoji,label]) => <button key={label} type="button" onClick={() => { setActiveCategory(label.toLowerCase()); setGarimparTab('buscar'); void loadProducts(false, true); }} className="rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-bold text-slate-600 hover:border-orange-300">{emoji} {label}</button>)}</div>}
