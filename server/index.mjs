...
async function handleOfferCopy(req, res) {
  ...
  const prompt = `Crie uma copy curta e diferente para WhatsApp para divulgar este produto. Variação ${nonce}. A mensagem DEVE começar exatamente com "🚨 RADAR ENCONTROU!" e seguir esta ordem: nome do produto; preço antigo riscado com ~; desconto real; preço atual em *negrito*; "🏷️ Oferta na Shopee"; "🛍️ Compre aqui"; link. Produto: ${name}. Descrição: ${description}. Preço antigo: ${originalPrice || 'não informado'}. Desconto real: ${discount || 'não informado'}. Preço atual: ${price || 'não informado'}. Link: ${link}. Copy anterior (NÃO REPETIR): ${previous || 'nenhuma'}. Varie somente uma frase curta de oportunidade, sem inventar prazo, estoque ou benefícios. NÃO escreva "EU AMEIIII", "pacote de vídeo" ou qualquer texto sem relação com o produto. NÃO inclua comissão, percentual de comissão, segredos ou dados administrativos. Retorne somente o texto final, com marcação WhatsApp *negrito* e ~riscado~.`;
  const controller = new AbortController();
