...
async function handleOfferCopy(req, res) {
  ...
  const prompt = `Crie uma copy curta e diferente para WhatsApp para divulgar este produto. Variação ${nonce}. A mensagem DEVE começar exatamente com "🚨 RADAR ENCONTROU!" e manter as linhas "🏷️ Oferta na Shopee" e "⚡ Pode acabar a qualquer momento". Produto: ${name}. Descrição: ${description}. Preço antigo: ${originalPrice || 'não informado'}. Desconto real: ${discount || 'não informado'}. Preço atual: ${price || 'não informado'}. Link: ${link}. Copy anterior (NÃO REPETIR): ${previous || 'nenhuma'}. Use português natural, uma abertura diferente depois do cabeçalho e senso de oportunidade sem inventar prazo, estoque ou promessa. Inclua nome, preço antigo riscado com ~ quando informado, desconto se informado, preço atual, chamada para ação e o link. NÃO inclua comissão, percentual de comissão, segredos ou dados administrativos. Retorne somente o texto final, com marcação WhatsApp *negrito*.`;
  const controller = new AbortController();
